...
export const handler = async (event) => {
  try {
    ...
    if (event.httpMethod === 'GET') {
      ...
      let query = supabase.from('todos').select('*').eq('owner_id', OWN).order('created_at', { ascending:false }).limit(limit)
    ...
    if (event.httpMethod === 'POST') {
      ...
      const { data, error } = await supabase
        .from('todos')
        .insert([{ owner_id: OWN, title, due_at, status:'open', source:'manual', case_id:null }])
        .select('id')
        .single()
    ...
    if (event.httpMethod === 'PATCH') {
      ...
      if (typeof body.status === 'string') patch.status = body.status.trim().toLowerCase()
      if (Object.prototype.hasOwnProperty.call(body,'due_at')) patch.due_at = body.due_at ?? null

      if (!Object.keys(patch).length) return json(400, { error:'no fields to update' })

      const { error } = await supabase.from('todos').update(patch).eq('owner_id', OWN).eq('id', id)
    ...
    if (event.httpMethod === 'DELETE') {
      ...
      const { error } = await supabase.from('todos').delete().eq('owner_id', OWN).eq('id', id)
