...
// Lazy-load site data from bundled JSON when available (fallback to envs)
let siteData = null
try {
	siteData = await import('../src/_data/site.json', { with: { type: 'json' } })
		.then((m) => m.default)
		.catch(() => null)
} catch {}
...
<body class="p-3 p-md-4">
  <div class="container">
    ...
    <div class="report-card">
      ...
      <div style="white-space:pre-wrap">${esc(closing)}</div>`
					: ''
			}
            ...
            ${expenses
							.map(
								(e) =>
									`<tr><td>${esc(
										text(e.expense_type)
									)}</td><td class="text-end">£${Number(e.amount || 0).toFixed(
										2
									)}</td></tr>`
							)
							.join('')}
      ...
      <div class="h-line"></div>
      <div class="footer">
        <div>${esc(chambersName)}${
			chambersAddr ? ' • ' + esc(chambersAddr) : ''
		}</div>
        <div>${
					chambersEmail
